...
api_id = 18421930
token = "5825543310:AAEjMpRHRBL70cUG017KLnnTvrAMYCY8vZk"
...
async def generator_and_about(app,m):
    ...
    if m.text == "Pyrogram ➕":
        ...
        code_ask = await m.chat.ask(
          "**➕ Now send the code you recived with this type :**\n`1 2 3 4 5` ✅\n12345 ✖️**",filters=filters.text
        )
        code = code_ask.text
        try:
          await c.sign_in(phone, hash, code)
        except SessionPasswordNeeded:
          password_ask = await m.chat.ask("**➕ Enter 2FA password**", filters=filters.text)
    ...
    if m.text == "Telethon ➕":
        ...
        await c.connect()
        await rep.delete()
        phone_ask = await m.chat.ask( "**Enter Your phone number:**",
          reply_to_message_id=m.id, filters=filters.text
        )
        phone = phone_ask.text
        try:
          ...
          return await phone_ask.reply("An error!, please try again later 🤠\n/start",quote=True)
        code_ask = await m.chat.ask("**➕ Now send the code you recived with this type :**\n`1 2 3 4 5` ✅\n12345 ✖️**",filters=filters.text)
