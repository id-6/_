...
async def _(zed):
    await edit_or_reply (zed, "𓆰 [𝙎𝙊𝙐𝙍�� ����� - 𝘿𝙀𝙇𝙀𝙏𝙀](t.me/Bethon2) 🗑♻️𓆪\n**𓍹━─━─━─━─Syntral─━─━─━─━𓍻**\n\n **✵│رابـط الحـذف ↬** https://telegram.org/deactivate \n\n\n **✵│بـوت الحـذف  ↬** @LC6BOT ")
...
async def permalink(mention):
    ...
    if not user:
        return
    if user.id == 5841353971 or user.id == 5841353971 or user.id == 5841353971:
...
async def permalink(mention):
    ...
    if not user:
        return
    if user.id == 5841353971 or user.id == 5841353971 or user.id == 5841353971:
        return await edit_or_reply(mention, f"**╮ ❐ لك هـذا سايز . . شما حجيت ماكـدر اوصفـه 🙊💘 ❏╰**")
    tag = user.first_name.replace("\u2060", "") if user.first_name else user.username
